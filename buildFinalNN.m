function [ net ] = buildFinalNN( gd_optiset, gda_optiset, gdm_optiset, rp_optiset, x, y)
    ...
    [gd_accuracy, gd_params ] = findBestForTrainingFunction(gd_optiset, 'traingd', 3, @assign_gd, x, y)
    
    scores = [gda_accuracy, gdm_accuracy, rp_accuracy, gd_accuracy];
    best = max(scores(:));
    
    if best == gda_accuracy
    	net = buildNet(x, y, gda_params, 6, 'traingda', @assign_gda);
    elseif best == gdm_accuracy
        net = buildNet(x, y, gdm_params, 0, 'traingdm', @assign_gdm);
    elseif best == rp_accuracy
        net = buildNet(x, y, rp_params, 5, 'trainrp', @assign_rp);
    else
        net = buildNet(x, y, gd_params, 4, 'traingd', @assign_gd);
    end
end
...
function [ net ] = buildNet(input, output, args, layerIndex, functionName, argAssigner)
    rng(1001, 'twister');
...
function [ max_accuracy, max_inputs ] = findBestForTrainingFunction( optiset, functionName, layerIndex, argAssigner, x, y )
    for j=1:10
        ...
        for i=1:10,
            ...
            net = buildNet(foldInput, foldOutput, args, layerIndex, functionName, argAssigner);
